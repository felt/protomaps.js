...
function minHeap() {
    ...
    heap.remove = function (removed) {
        var i = removed.index, object = array.pop();
        if (i !== array.length) {
    ...
    function up(i) {
        var object = array[i];
        while (i > 0) {
            var up = ((i + 1) >> 1) - 1, parent = array[up];
    ...
    function down(i) {
        var object = array[i];
        while (true) {
            var right = (i + 1) * 2, left = right - 1, down = i, child = array[down];
...
const area = (t) => {
    return Math.abs((t[0].x - t[2].x) * (t[1].y - t[0].y) -
        (t[0].x - t[1].x) * (t[2].y - t[0].y));
};
export const simplify = (rings, pointsToKeep) => {
    let globalWeights = [];
...
const simplifyRing = (ring, pointsToKeep) => {
    var maxArea = 0;
    let heap = minHeap();
    var triangle;
    let triangles = [];
    ...
    var weights = ring.map(function (d) {
        return d.area > 0 ? d.area : Infinity; /*+= Math.random(); /* break ties */
    });
    return weights;
};
export const splitMultiLineString = (mls, maxVertices) => {
    let retval = [];
    var current = [];
...
export const splitMultiPolygon = (mp, maxVertices) => {

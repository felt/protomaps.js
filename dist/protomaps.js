  ...
  var LineSymbolizer = class {
    ...
    before(ctx, z2) {
      if (!this.per_feature) {
        ...
        this.skip = width === 0 || this.dash !== null && this.dashWidth.get(z2) === 0;
        ctx.lineWidth = width;
        ctx.globalAlpha = this.opacity.get(z2);
    ...
    draw(ctx, geom, z2, f2) {
      if (this.skip)
        return;
      const widthValue = this.width.get(z2, f2);
      if (widthValue === 0 || this.dash !== null && this.dashWidth.get(z2, f2) === 0)
        return;
      const setStyle = () => {
        if (this.per_feature) {
          ctx.globalAlpha = this.opacity.get(z2, f2);
    ...
    class LeafletLayer extends L.GridLayer {
      ...
      updateDataSources(dataSources, paintRules2, labelRules2) {
        ...
        dataSources.forEach((d) => {
          ...
          if (!exists || exists && this.views.get(d.name).tileCache.source.url !== d.options.url) {
